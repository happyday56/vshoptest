...
function GotoNext() {
    ...
    //var result = { id: selectedCategoryId, name: $("#fullName").html() };
    $("#categoryId").val(selectedCategoryId);
    $("#categoryPath").val($("#fullName").html());
    //alert(parent.$("#ctl00_contentHolder_btnCategories").val());
    //alert(parent.$("#spCategories").html());

    var categoryIds = parent.$("#ctl00_contentHolder_btnCategories").val();
    var exist = false;
    for (var i = 0; i < categoryIds.split(',').length ; i++) {
        if (categoryIds.split(',')[i] == selectedCategoryId) {
            exist = true;
            break;
        }
    }
    if (!exist) {
        parent.$("#ctl00_contentHolder_btnCategories").val(parent.$("#ctl00_contentHolder_btnCategories").val().length == 0 ? selectedCategoryId : parent.$("#ctl00_contentHolder_btnCategories").val() + ',' + selectedCategoryId);
        parent.$("#spCategories").append('<div>' + $("#fullName").html() + ' <input type="button" value="-" onclick="removeCategory(this,' + selectedCategoryId + ')"/>' + '</div>');
    }
...
$(document).ready(function () {
    ...
    closeButton.bind("click", function () { GotoClose(); return false; });
